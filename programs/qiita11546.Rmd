...
saveRDS(output, "../outputs/qiita_11546/res_ancombc2.rds")
```

```{r}
output = read_rds(file = "../outputs/qiita_11546/res_ancombc2.rds")
...
write_csv(df_zero, "../outputs/qiita_11546/res_presence_absence.csv")
```
...
fig_sens = tab_sens %>%
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5),
        legend.position = "top")
fig_sens
ggsave("../figures/supp/qiita_11546_sens.jpeg", width = 8, height = 8, dpi = 100)
ggsave("../figures/supp/qiita_11546_sens.pdf", width = 8, height = 8)
```
...
fig_heat_pair
ggsave("../figures/main/qiita_11546.jpeg", width = 6.25, height = 5, dpi = 100)
ggsave("../figures/main/qiita_11546.pdf", width = 6.25, height = 5)
```
...
df_fig0 = res_pair %>%
    ...
    transmute(species,
              lfc1 = lfc_surgery_typeileocolonic,
              lfc2 = lfc_surgery_typecolectomy,
              lfc3 = lfc_surgery_typecolectomy_surgery_typeileocolonic,
              se1 = se_surgery_typeileocolonic,
              se2 = se_surgery_typecolectomy,
              se3 = se_surgery_typecolectomy_surgery_typeileocolonic) 

df_fig1 = df_fig0 %>%
  transmute(species, g1 = lfc1, g2 = lfc2, g3 = lfc3) %>%
  pivot_longer(cols = g1:g3, 
               names_to = "group", values_to = "lfc") %>%
  arrange(species)

df_fig2 = df_fig0 %>%
  transmute(species, g1 = se1, g2 = se2, g3 = se3) %>%
  pivot_longer(cols = g1:g3, 
               names_to = "group", values_to = "se") %>%
  arrange(species)
...
df_color = res_pair %>%
    filter(p_surgery_typecolectomy < 0.05 | 
             p_surgery_typeileocolonic < 0.05 |
             p_surgery_typecolectomy_surgery_typeileocolonic < 0.05) %>%
    transmute(species,
              color = ifelse(q_surgery_typecolectomy < 0.05 |
                               q_surgery_typeileocolonic < 0.05 |
                               q_surgery_typecolectomy_surgery_typeileocolonic < 0.05, 
                             "tan3", "black")) %>%
    arrange(species)

fig_bar_pair = df_fig %>%
  ggplot(aes(x = species, y = lfc, fill = group)) +
  ...
  scale_fill_jco(name = NULL, 
                 labels = c("Ileocolonic - None", 
                            "Colectomy - None", 
                            "Colectomy - Ileocolonic")) +
  theme_pubr() +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(colour = df_color$color,
                                   angle = 60, hjust = 0.5, vjust = 0.5))

fig_bar_pair
ggsave("../figures/main/qiita_11546.jpeg", width = 10, height = 5, dpi = 100)
ggsave("../figures/main/qiita_11546.pdf", width = 10, height = 5)
```
