                  ...
                  .packages = c("ANCOMBC", "tidyverse")) %dorng% 
  {
    ...
    res_merge1 = res_trend %>%
      ...
      dplyr::left_join(fmd %>%
                       ...
                       by = "taxon")
    diff_est = res_merge1$diff_est
    diff_true = res_merge1$diff_true
    ...
    res_merge2 = res_trend %>%
      dplyr::transmute(taxon, 
                       diff_est = diff_abn * passed_ss) %>%
      dplyr::left_join(fmd %>%
                         dplyr::transmute(taxon, 
                                          diff_true = ifelse(lfc_cat2_vs_1 == 0, 0, 1)),
                       by = "taxon")
    diff_est = res_merge2$diff_est
    diff_true = res_merge2$diff_true
...
write_csv(data.frame(res_sim), "qmp_sim_trend_ancombc2.csv")
